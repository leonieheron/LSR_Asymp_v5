...
author: "Diana Buitrago-Garcia, Aziz Mert Ipekci, Leonie Heron, Hira Imeri, Georgia Salanti, Nicola Low, ..."
...
asymptomaticQ1$label[asymptomaticQ1$label == "#4479 Harada S [cluster:1]"] <- "#4479 Harada S [Patients]"
asymptomaticQ1$label[asymptomaticQ1$label == "#4479 Harada S [cluster:2]"] <- "#4479 Harada S [Healthcare workers]"
asymptomaticQ1$label[asymptomaticQ1$label == "#2892 Kennelly SP [cluster:1]"] <- "#2892 Kennelly SP [Nursing home residents]"
asymptomaticQ1$label[asymptomaticQ1$label == "#2892 Kennelly SP [cluster:2]"] <- "#2892 Kennelly SP [Nursing home staff]"
asymptomaticQ1$label[asymptomaticQ1$label == "#4968 van Buul LW [cluster:1]"] <- "#4968 van Buul LW [Nursing home residents]"
asymptomaticQ1$label[asymptomaticQ1$label == "#4968 van Buul LW [cluster:2]"] <- "#4968 van Buul LW [Healthcare workers]"
asymptomaticQ1$label[asymptomaticQ1$label == "#9442 Theuring S [cluster:1]"] <- "#9442 Theuring S [School students and staff]"
asymptomaticQ1$label[asymptomaticQ1$label == "#9442 Theuring S [cluster:2]"] <- "#9442 Theuring S [Household members]"
asymptomaticQ1$label[asymptomaticQ1$label == "#7465 van den Besselaar JH [cluster:1]"] <- "#7465 van den Besselaar JH [Healthcare workers]"
asymptomaticQ1$label[asymptomaticQ1$label == "#7465 van den Besselaar JH [cluster:2]"] <- "#7465 van den Besselaar JH [Residents]"
asymptomaticQ1$label[asymptomaticQ1$label == "#2826 Taylor J [cluster:1]"] <- "#2826 Taylor J [Healthcare personnel]"
asymptomaticQ1$label[asymptomaticQ1$label == "#2826 Taylor J [cluster:2]"] <- "#2826 Taylor J [Residents]"
asymptomaticQ1$label[asymptomaticQ1$label == "#3719 Ladhani SN [cluster:1]"] <- "#3719 Ladhani SN [Residents]"
asymptomaticQ1$label[asymptomaticQ1$label == "#3719 Ladhani SN [cluster:2]"] <- "#3719 Ladhani SN [Healthcare workers]"
...
png(file = 'forest_meta_Q1.png',width=28,height=70, res=600, units="cm") 
...
pdf(file = 'forest_meta_Q1.pdf', width = 10, height = 28) 
...
p=ggplot()+ 
  ...
  geom_text(aes(y=c(-1,-1,-1), x=c(-1.5,1.25,1.75), label=c("Study","Prop.","95% CI")), hjust = 0, fontface = "bold")+
  ...
  geom_text(data=dataG,aes(y=dataG$line, x=-1.5, label=dataG$label), hjust = 0, fontface=dataG$fontface)+
...
# two_models$author_1 <- gsub(",","",two_models$author_1)
...
# L1$label <- "Value"
# for (i in 1:nrow(L1)) {
#   if (L1$model[i] == "q3_pp_m_1") {
#   L1$label[i] <- paste(L1$author_1[i], " [", L1$q3_setting_1[i], "]", sep = "")
#   }else{
#     L1$label[i] <- paste(L1$author_1[i], " [", L1$q3_setting_2[i], "]", sep = "")
#   }
#   
# }
# 
# L1$label2 <- paste(L1$author_1, " [", substrRight(L1$model, 1), "]", sep = "")
# L2 <- two_models %>%
#   pivot_longer(cols = c(8:9),
#                names_to = "model",
#                values_to = "q3_pp_l") %>%
#   select("record_id", "model", "q3_pp_l", "author_1", "q3_setting_1", "q3_setting_2")
# 
# L2$label2 <- paste(L2$author_1, " [", substrRight(L2$model, 1), "]", sep = "")
# L2$label <- "Value"
# i = 1
# for (i in 1:nrow(L2)) {
#   if (L2$model[i] == "q3_pp_l_1") {
#     sett -> L2$q3_setting_1[i]
#   }else{
#     sett -> L2$q3_setting_2[i]
#   }
#   L2$label[i] <- paste(L2$author_1[i], " [", sett, "]", sep = "")
# }
# 
# i=1
# for (i in 1:nrow(L2)) {
#   if (L2$model[i] == "q3_pp_l_1") {
#     L2$label[i] <- paste(L2$author_1[i], " [", L2$q3_setting_1[i], "]", sep = "")
#   }else{
#     L2$label[i] <- paste(L2$author_1[i], " [", L2$q3_setting_2[i], "]", sep = "")
#   }
#   
# }
# 
...
#                names_to = "model",
#                values_to = "q3_pp_u") %>%
#   select("record_id", "model", "q3_pp_u", "author_1", "q3_setting_1", "q3_setting_2")
# 
# L3$label <- "Value"
# i = 1
# for (i in 1:nrow(L3)) {
#   if (L3$model[i] == "q3_pp_u_1") {
#     L3$label[i] <- paste(L3$author_1[i], " [", L3$q3_setting_1[i], "]", sep = "")
#   }else{
#     L3$label[i] <- paste(L3$author_1[i], " [", L3$q3_setting_2[i], "]", sep = "")
#   }
#   
# }
# 
# L3$label2 <- paste(L3$author_1, " [", substrRight(L3$model, 1), "]", sep = "")
# 
...
# L4$label2 <- paste(L4$author_1, " [", substrRight(L4$model, 1), "]", sep = "")
# L4$label <- paste(L4$author_1, " [", L4$setting, "]", sep = "")
# 
